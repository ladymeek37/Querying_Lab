...
def problem_one(request):
    ...
    students_gpa3 = Student.objects.filter(gpa__gte = 3.0).order_by('-gpa')

    for student in students_gpa3:
...
def problem_five(request):
    ...
    lady = Student.objects.create(first_name = "Lady", last_name = "Meek", year = 12, gpa = 4.0)

    print(f'Id: {lady.id}')
    print(f'Full_name: {lady.first_name} {lady.last_name}')
    print(f'Year: {lady.year}')
    print(f'GPA: {lady.gpa}')
...
def problem_six(request):
    ...
    student_id = 11
    Student.objects.filter(id = 11).update(gpa = 3.8)
    lady = Student.objects.get(pk = 11)
    print (f'Id: {lady.id}')
    print (f'Full Name: {lady.first_name} {lady.last_name}')
    print (f'GPA: {lady.gpa}')
    return complete(request)
